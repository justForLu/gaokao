...
class FileController extends BaseController
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function uploadCarImg(Request $request)
    {
        $file = $_FILES['file'];
        $savePath = Config::get('api.uploadCarImg').'/'.date('Y-m').'/'.date('d');
        $filename = date('YmdHis',time()) . mt_rand(100000, 999999);
        $file_size = 500;
    ...
    {
        $file = $_FILES['file'];
        $savePath = Config::get('api.uploadCodeImg').'/'.date('Y-m').'/'.date('d');
        $filename = date('YmdHis',time()) . mt_rand(100000, 999999);
        $file_size = 800;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function uploadBillImg(Request $request)
    {
        $file = $_FILES['file'];
        $savePath = Config::get('api.uploadBeanImg').'/'.date('Y-m').'/'.date('d');
        $filename = date('YmdHis',time()) . mt_rand(100000, 999999);
        $file_size = 800;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function uploadBeanImg(Request $request)
    {
        $file = $_FILES['file'];
        $savePath = Config::get('api.uploadBeanImg').'/'.date('Y-m').'/'.date('d');
        $filename = date('YmdHis',time()) . mt_rand(100000, 999999);
